...
class PackageManager
{
    ...
    {
        if (isWindows()) {
            return !in_array("'".$formula."' not found", explode(PHP_EOL, $this->cli->run('scoop which '.$formula)));
        } else {
    ...
    function installOrFail($formula, array $taps = [])
    {
        ...
        if (isWindows()) {
            $command = 'scoop install '.$formula;
        } else {
            $command = 'brew install '.$formula;
        }
        $this->cli->run($command, function ($errorOutput) use ($formula) {
            output($errorOutput);
